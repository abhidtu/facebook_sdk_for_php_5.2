 ...
 */
class Facebook_FacebookRedirectLoginHelper
{
  ...
  {
    $this->appId = Facebook_FacebookSession::_getTargetAppId($appId);
    $this->appSecret = Facebook_FacebookSession::_getTargetAppSecret($appSecret);
  ...
  {
    //$version = ($version ?: Facebook_FacebookRequest::GRAPH_API_VERSION);
    ...
    {
       $version = Facebook_FacebookRequest::GRAPH_API_VERSION;
    }
    ...
    $params = array(
      ...
      'state' => $this->state,
      'sdk' => 'php-sdk-' . Facebook_FacebookRequest::VERSION,
      'scope' => implode(',', $scope)
    );
  ...
  public function getSessionFromRedirect()
  {
    ...
    if ($this->isValidRedirect()) {
      $params = array(
        'client_id' => Facebook_FacebookSession::_getTargetAppId($this->appId),
        'redirect_uri' => $this->redirectUrl,
        'client_secret' =>
          Facebook_FacebookSession::_getTargetAppSecret($this->appSecret),
        'code' => $this->getCode()
      );
      $responsemy = (new Facebook_FacebookRequest(
        Facebook_FacebookSession::newAppSession($this->appId, $this->appSecret),
        'GET',
        '/oauth/access_token',
        $params
      ));
      $response=$responsemy->execute()->getResponse();
      if (isset($response['access_token'])) {
        return new Facebook_FacebookSession($response['access_token']);
      }
    }
    return null;
  }
