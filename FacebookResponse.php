 ...
 */
class Facebook_FacebookResponse
{
   ...
   * @return mixed
   */
  public function getGraphObject($type = 'Facebook_GraphObject') {
      $temp=new Facebook_GraphObject($this->responseData);
   ...
   * @return mixed
   */
  public function getGraphObjectList($type = 'Facebook_GraphObject') {
    $out = array();
    $data = $this->responseData->data;
    for ($i = 0; $i < count($data); $i++) {
        $temp1=new Facebook_GraphObject($data[$i]);
        $out[] = $temp1->cast($type);
    }
    return $out;
  }
  ...
  private function handlePagination($direction) {
    if (isset($this->responseData->paging->$direction)) {
      ...
      return new Facebook_FacebookRequest(
