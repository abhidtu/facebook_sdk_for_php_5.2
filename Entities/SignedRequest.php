 ...
 * @package Facebook
 */
class Facebook_Entities_SignedRequest
{
  ...
  public function __construct($rawSignedRequest = null, $state = null, $appSecret = null)
  {
    ...
    $this->payload = self::parse($rawSignedRequest, $state, $appSecret);
  }
  ...
  public static function make(array $payload, $appSecret = null)
  {
    ...
    $hashedSig = self::hashSignature($encodedPayload, $appSecret);
    $encodedSig = self::base64UrlEncode($hashedSig);
  ...
  {
    list($encodedSig, $encodedPayload) = self::split($signedRequest);

    // Signature validation
    $sig = self::decodeSignature($encodedSig);
    $hashedSig = self::hashSignature($encodedPayload, $appSecret);
    self::validateSignature($hashedSig, $sig);

    // Payload validation
    $data = self::decodePayload($encodedPayload);
    self::validateAlgorithm($data);
    if ($state) {
      self::validateCsrf($data, $state);
    }
  ...
  {
    self::validateFormat($signedRequest);
  ...
  public static function base64UrlDecode($input)
  {
    ...
    self::validateBase64($urlDecodedBase64);
