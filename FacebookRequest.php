 ...
 */
class Facebook_FacebookRequest
{
  ...
  {
    self::$httpClientHandler = $handler;
  }
  ...
  {
    if (self::$httpClientHandler) {
      return self::$httpClientHandler;
    }
    return function_exists('curl_init') ? new Facebook_HttpClients_FacebookCurlHttpClient() : new Facebook_HttpClients_FacebookStreamHttpClient();
  }
  ...
  public function __construct(
    Facebook_FacebookSession $session, $method, $path, $parameters = null, $version = null, $etag = null
  )
  {
    ...
    if ($version) {
      ...
      $this->version = self::GRAPH_API_VERSION;
    }
    $this->etag = $etag;
    ...
    if (Facebook_FacebookSession::useAppSecretProof()
  ...
  {
    return self::BASE_GRAPH_URL . '/' . $this->version . $this->path;
  }
  ...
  public function execute()
  {
    ...
    if ($decodedResult === null) {
      ...
      return new Facebook_FacebookResponse($this, $out, $result, $etagHit, $etagReceived);
    }
    if (isset($decodedResult->error)) {
      throw Facebook_FacebookRequestException::create(
        $result,
        $decodedResult->error,
    ...
    return new Facebook_FacebookResponse($this, $decodedResult, $result, $etagHit, $etagReceived);
  }
  ...
  {
    return hash_hmac('sha256', $token, Facebook_FacebookSession::_getTargetAppSecret());
  }
