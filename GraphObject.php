...
class Facebook_GraphObject
{
  ...
  public function cast($type)
  {
    ...
    if (is_subclass_of($type, Facebook_GraphObject::className())) {
   ...
   * @return mixed
   */
  public function getProperty($name, $type = 'Facebook_GraphObject')
  {
    if (isset($this->backingData[$name])) {
      ...
      if (is_scalar($value)) {
        return $value;
          ...
          $temp3=new Facebook_GraphObject($value);
          return ($temp3->cast($type));
      }
    } else {
      return null;
    }
   ...
   * @return array
   */
  public function getPropertyAsArray($name, $type = 'Facebook_GraphObject')
  {
    $target = array();
    ...
    foreach ($target as $key => $value) {
      if (is_scalar($value)) {
        $out[$key] = $value;
          ...
          $temp4=new Facebook_GraphObject($value);
        $out[$key] = $temp4->cast($type);
      }
    }
    return $out;
  }
